...
namespace Unity.Cloud.UserReporting.Client
{
    ...
    public class UserReportingClient
    {
                                ...
                                try
                                {
                                    ...
                                    if (userReportResult != null)
                                    {
                                        if (SendEventsToAnalytics)
                                        {
                                            ...
                                            Platform.SendAnalyticsEvent("UserReportingClient.SendUserReport",
                                                eventData);
                                        }
        ...
        public void Update()
        {
            ...
            // Synchronization
            foreach (var synchronizedAction in synchronizedActions) currentSynchronizedActions.Add(synchronizedAction);
        ...
        private void WaitForPerforation(int currentScreenshotsTaken, Action callback)
        {
            if (screenshotsSaved >= currentScreenshotsTaken && isMeasureBoundary)
                ...
                synchronizedActions.Add(() => { WaitForPerforation(currentScreenshotsTaken, callback); });
        }
