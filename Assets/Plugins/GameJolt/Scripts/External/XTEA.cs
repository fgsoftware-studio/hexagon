...
namespace GameJolt.External
{
	...
	///     This algorithm is not as secure as AES or TripleDES, but because of it's small footprint it's a good choise for
	///     mobile applications.
	/// </summary>
    ...
    public static class XTEA
    {
        ...
        private static uint[] CreateKey(byte[] key)
        {
            ...
            for (var i = 0; i < key.Length; i++) hash[i % 16] = (byte) ((31 * hash[i % 16]) ^ key[i]);
            for (var i = key.Length; i < hash.Length; i++) // if key was too short
