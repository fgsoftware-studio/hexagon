...
namespace GameJolt.API
{
	...
	public class GameJoltAPI : MonoSingleton<GameJoltAPI>
    {
        ...
        [Tooltip("Event for the autologin mechanism.")]
        ...
        /// </summary>
        public bool HasSignedInUser => HasUser && currentUser.IsAuthenticated;
        ...
        private IEnumerator AutoConnect()
        {
            ...
            if (GetStoredUserCredentials(out username, out token))
            {
                ...
                user.SignIn(
                    success => AutoLoginEvent.Invoke(success ? AutoLoginResult.Success : AutoLoginResult.Failed));
            }
            else
            {
                AutoLoginEvent.Invoke(AutoLoginResult.MissingCredentials);
        ...
        {
            if (Settings.UseCaching) Scores.GetTables(null);
        }
        ...
        public bool RememberUserCredentials(string username, string token)
        {
            ...
            var credentials = XTEA.Encrypt(username, Settings.EncryptionKey) + "#" +
                              XTEA.Encrypt(token, Settings.EncryptionKey);
