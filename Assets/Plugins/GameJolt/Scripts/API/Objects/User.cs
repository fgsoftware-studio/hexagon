...
namespace GameJolt.API.Objects
{
	...
	public sealed class User : Base
    {
        ...
        public string Name
        {
            ...
            set
            {
                if (name.ToLower() != value.ToLower()) IsAuthenticated = false;
                name = value;
            }
        }
        ...
        public string Token
        {
            ...
            set
            {
                if (token.ToLower() != value.ToLower()) IsAuthenticated = false;
                token = value;
            }
        }
        ...
        /// </summary>
        public bool IsOnline => LastLoggedIn == "Online Now";
        ...
        ///     Signs in the user.
        /// </summary>
        /// <param name="signedInCallback">
        ///     A callback function accepting a single parameter, a boolean indicating whether the user
        ///     has been signed-in successfully.
        /// </param>
        /// <param name="userFetchedCallback">
        ///     A callback function accepting a single parameter, a boolean indicating whether the
        ///     user's information have been fetched successfully.
        /// </param>
            ...
            bool rememberMe = false)
        {
            if (GameJoltAPI.Instance.HasUser)
            {
                ...
                if (signedInCallback != null) signedInCallback(false);
                if (userFetchedCallback != null) userFetchedCallback(false);

                return;
            }

            var parameters = new Dictionary<string, string>
                {{"username", Name.ToLower()}, {"user_token", Token.ToLower()}};
            Request.Get(Constants.ApiUsersAuth, parameters, response =>
            {
                ...
                if (response.Success)
                {
                    if (GameJoltAPI.Instance.Settings.AutoSignInOutMessage)
                        GameJoltUI.Instance.QueueNotification(string.Format(GameJoltAPI.Instance.Settings.SignInMessage,
                            Name));
                    GameJoltAPI.Instance.CurrentUser = this;

                    if (rememberMe) GameJoltAPI.Instance.RememberUserCredentials(Name, Token);

                    if (signedInCallback != null) signedInCallback(true);

                    Get(user =>
                    {
                        if (userFetchedCallback != null) userFetchedCallback(user != null);
                    });
                }
                else
                {
        ...
        public void SignOut()
        {
            ...
            {
                if (GameJoltAPI.Instance.Settings.AutoSignInOutMessage)
                    GameJoltUI.Instance.QueueNotification(string.Format(GameJoltAPI.Instance.Settings.SignOutMessage,
                        Name));
        ...
        public void DownloadAvatar(Action<bool> callback = null)
        {
            if (!string.IsNullOrEmpty(AvatarURL))
            {
                Misc.DownloadImage(AvatarURL, avatar =>
                {
                    ...
                    if (callback != null) callback(avatar != null);
                });
            }
            else
            {
