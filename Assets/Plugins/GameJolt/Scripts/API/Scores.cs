...
namespace GameJolt.API
{
	...
	public static class Scores
    {
        ...
        public static void GetRank(int value, int table = 0, Action<int> callback = null)
        {
            ...
            Request.Get(Constants.ApiScoresRank, parameters, response =>
            {
                if (callback != null)
                {
        ...
        public static void GetTables(Action<Table[]> callback)
        {
            ...
            {
                if (callback != null) callback(cachedTables);
            }
            else
            {
                Request.Get(Constants.ApiScoresTablesFetch, null, response =>
                {
                    Table[] tables;
                    if (response.Success)
                    {
                        var count = response.Json["tables"].AsArray.Count;
                        tables = new Table[count];

                        for (var i = 0; i < count; ++i) tables[i] = new Table(response.Json["tables"][i].AsObject);
                    }
                    else
                    {
                        tables = null;
                    ...
                    if (GameJoltAPI.Instance.Settings.UseCaching) cachedTables = tables;

                    if (callback != null) callback(tables);
                }, false);
            }
        }
            ...
            Action<bool> callback = null)
        {
            ...
            if (guestScore) parameters.Add("guest", guestName);
            if (table != 0) parameters.Add("table_id", table.ToString());
            if (!string.IsNullOrEmpty(extra)) parameters.Add("extra_data", extra);

            Request.Get(Constants.ApiScoresAdd, parameters, response =>
            {
                if (callback != null) callback(response.Success);
            }, !guestScore);
        }
        ...
        ///     If set to <c>true</c> only return scores of the current user; otherwise for all the
        ///     users.
        /// </param>
        ...
        ///     users.
        /// </param>
        public static void GetBetterThan(int score, Action<Score[]> callback, int table = 0, int limit = 10,
        ...
        /// <param name="table">The ID of the HighScore <see cref="Table" />. Defaults to 0 (i.e. Primary Table).</param>
        /// <param name="limit">The maximum number of <see cref="Score" />s to return. Defaults to 10.</param>
        public static void GetBetterThan(string guest, int score, Action<Score[]> callback, int table = 0,
            int limit = 10)
        {
        ...
        ///     users.
        /// </param>
        public static void GetWorseThan(int score, Action<Score[]> callback, int table = 0, int limit = 10,
            bool currentUserOnly = false)
        {
        ...
        /// <param name="table">The ID of the HighScore <see cref="Table" />. Defaults to 0 (i.e. Primary Table).</param>
        /// <param name="limit">The maximum number of <see cref="Score" />s to return. Defaults to 10.</param>
        public static void GetWorseThan(string guest, int score, Action<Score[]> callback, int table = 0,
            int limit = 10)
        {
            ...
            int? betterThan, int? worseThan)
        {
            ...
            if (!currentUserOnly && guest != "") parameters.Add("guest", guest);

            Request.Get(Constants.ApiScoresFetch, parameters, response =>
            {
                Score[] scores;
                if (response.Success)
                {
                    var count = response.Json["scores"].AsArray.Count;
                    scores = new Score[count];

                    for (var i = 0; i < count; ++i) scores[i] = new Score(response.Json["scores"][i].AsObject);
                }
                else
                {
                    scores = null;
                ...
                if (callback != null) callback(scores);
            }, currentUserOnly);
        }
