...
namespace GameJolt.API
{
	...
	public static class Misc
    {
        ...
        private static readonly Dictionary<string, Action<Sprite>> OpenRequests =
            new Dictionary<string, Action<Sprite>>();
        ...
        {
            foreach (var sprite in SpriteCache.Values) Object.Destroy(sprite);
            SpriteCache.Clear();
        }
        ...
        public static void DownloadImage(string url, Action<Sprite> callback)
        {
            ...
            else
            {
                ...
                GameJoltAPI.Instance.StartCoroutine(GameJoltAPI.Instance.GetRequest(url, ResponseFormat.Texture,
                    response =>
                    {
                        Sprite sprite = null;
                        if (response.Success)
        ...
        {
            Request.Get(Constants.ApiTimeGet, null, response =>
            {
                if (callback != null)
                {
                    var timestamp = response.Json["timestamp"].AsDouble;
                    var serverTime = new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified);
