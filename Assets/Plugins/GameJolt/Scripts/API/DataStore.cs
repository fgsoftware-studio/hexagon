...
namespace GameJolt.API
{
	...
	public static class DataStore
    {
	    ...
	    /// <param name="global">
	    ///     A boolean indicating whether the key is global (<c>true</c>) or private to the user (<c>false</c>
	    ///     ).
	    /// </param>
	    ...
	    /// <param name="callback">A callback function accepting a single parameter, a boolean indicating success.</param>
	    /// <param name="progress">
	    ///     A callback function accepting a single parameter, the number of already uploaded bytes.
	    ///     This callback is called after each successfull segment upload.
	    /// </param>
            ...
            Action<int> progress = null, int maxSegmentSize = SoftLimit)
        {
            ...
            var encodedKeySize = GetEncodedSize(key);
            ...
            var encodedSize = Segmentate(Encoding.ASCII.GetBytes(value), maxSegmentSize, segments);
            ...
            completedAction = response =>
            {
                ...
                else
                {
                    ...
                    if (dataSend >= value.Length)
                    {
                        // data uploaded completely
                        callback(true);
                    }
                    else
                    {
            ...
            Action<string> callback = null)
        {
            ...
            {
                if (callback != null) callback(response.Success ? response.Dump : null);
	    ...
	    /// </summary>
	    /// <param name="global">
	    ///     A boolean indicating whether the keys are global (<c>true</c>) or private to the user (
	    ///     <c>false</c>).
	    /// </param>
	    ...
	    public static void GetKeys(bool global, string pattern, Action<string[]> callback)
        {
            ...
            Request.Get(Constants.ApiDatastoreKeysFetch, parameters, response =>
            {
                ...
                if (response.Success)
                {
                    var count = response.Json["keys"].AsArray.Count;
                    keys = new string[count];

                    for (var i = 0; i < count; ++i) keys[i] = response.Json["keys"][i]["key"].Value;
                }
                else
                {
                    keys = null;
                ...
                if (callback != null) callback(keys);
            }, !global);
        }
        ...
        static DataStore()
        {
            for (var c = '\0'; c < 256; c++) // ASCII chars from 0-32 and 127-255 are invalid
        ...
        {
            var start = 0; // start of the current segment
            var capacity = 0; // url encoded size of the current segment
            var totalCapacity = 0; // url encoded size of the whole message
