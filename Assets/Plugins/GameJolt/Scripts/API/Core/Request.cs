...
namespace GameJolt.API.Core
{
	...
	public static class Request
    {
	    ...
	    /// <param name="requireVerified">Whether a signed in user is required <c>true</c> or not <c>false</c>.</param>
	    /// <param name="format">The <see cref="ResponseFormat" /> to receive the <see cref="Response" /> in.</param>
	    private static string Prepare(ref Dictionary<string, string> parameters, bool requireVerified,
            ResponseFormat format)
        {
            ...
            if (requireVerified)
            {
                if (!GameJoltAPI.Instance.HasSignedInUser) return "Missing Authenticated User.";
            ...
            Dictionary<string, string> payload)
        {
            ...
            var signature = GetSignature(sb.ToString());
	    ...
	    private static string Md5(string input)
        {
            ...
            foreach (var b in hashBytes) hashString.Append(b.ToString("x2").ToLower());
