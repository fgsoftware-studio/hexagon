...
namespace GameJolt.API
{
	...
	public static class Users
    {
        ...
        public static void Get(User user, Action<User> callback)
        {
            ...
            else if (user.ID != 0) parameters.Add("user_id", user.ID.ToString());

            Request.Get(Constants.ApiUsersFetch, parameters, response =>
            {
                if (response.Success)
        ...
        public static void Get(int[] ids, Action<User[]> callback)
        {
            var parameters =
                new Dictionary<string, string>
                    {{"user_id", string.Join(",", ids.Select(id => id.ToString()).ToArray())}};
            ...
            Request.Get(Constants.ApiUsersFetch, parameters, response =>
            {
                ...
                if (response.Success)
                {
                    var count = response.Json["users"].AsArray.Count;
                    users = new User[count];

                    for (var i = 0; i < count; ++i) users[i] = new User(response.Json["users"][i].AsObject);
                }
                else
                {
                    users = null;
