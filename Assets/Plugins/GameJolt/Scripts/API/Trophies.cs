...
namespace GameJolt.API
{
	...
	public static class Trophies
    {
        ...
        public static void Unlock(int id, Action<bool> callback = null)
        {
            ...
            Request.Get(Constants.ApiTrophiesAdd, parameters, response =>
            {
                // Update the cache.
        ...
        public static void TryUnlock(int id, Action<TryUnlockResult> callback = null)
        {
            Get(id, trophy =>
            {
                if (trophy == null)
                {
                    // something went wrong -> signal failure
                    if (callback != null) callback(TryUnlockResult.Failure);
                }
                else if (!trophy.Unlocked)
                {
                    // not unlocked yet, so unlock it now
                    var unlockCallback = callback == null
                        ? (Action<bool>) null
                        : success => callback(success ? TryUnlockResult.Unlocked : TryUnlockResult.Failure);
                    Unlock(id, unlockCallback);
                }
                else if (callback != null)
                {
                    // already unlocked, so if we have a callback, signal success
        ...
        private static void PrepareNotification(int id)
        {
            if (cachedTrophies != null && cachedTrophies.ContainsKey(id))
            {
                if (cachedTrophies[id].Image != null)
                    ...
                    cachedTrophies[id].DownloadImage(success => { ShowNotification(cachedTrophies[id]); });
            }
            else
            {
                Get(id, trophy =>
                {
                    if (trophy != null)
                        trophy.DownloadImage(success => { ShowNotification(trophy); });
                });
            }
        }
        ...
        public static void Get(int id, Action<Trophy> callback)
        {
            ...
            {
                if (callback != null) callback(cachedTrophies.Values.FirstOrDefault(t => t.ID == id));
            }
            else
            {
                var parameters = new Dictionary<string, string> {{"trophy_id", id.ToString()}};

                Request.Get(Constants.ApiTrophiesFetch, parameters, response =>
                {
                    var trophy = response.Success ? new Trophy(response.Json["trophies"][0].AsObject) : null;

                    if (callback != null) callback(trophy);
                });
            }
        }
            ...
            else
            {
                var parameters =
                    new Dictionary<string, string>
                        {{"trophy_id", string.Join(",", ids.Select(id => id.ToString()).ToArray())}};
        ...
        public static void Get(Action<Trophy[]> callback)
        {
            if (cachedTrophies != null)
            {
                if (callback != null) callback(cachedTrophies.Values.ToArray());
            }
            else
            {
                ...
                Get(parameters, trophies =>
                {
                    ...
                    if (callback != null) callback(trophies);
                });
            }
        }
        ...
        /// </summary>
        /// <param name="unlocked">
        ///     A boolean indicating whether to retrieve unlocked (<c>true</c>) or locked (<c>false</c>)
        ///     <see cref="Trophy" />s.
        /// </param>
        /// <param name="callback">A callback function accepting a single parameter, an array of <see cref="Trophy" />s.</param>
        ...
        private static void Get(Dictionary<string, string> parameters, Action<Trophy[]> callback)
        {
            Request.Get(Constants.ApiTrophiesFetch, parameters, response =>
            {
                ...
                if (response.Success)
                {
                    var count = response.Json["trophies"].AsArray.Count;
                    trophies = new Trophy[count];

                    for (var i = 0; i < count; ++i) trophies[i] = new Trophy(response.Json["trophies"][i].AsObject);
                }
                else
                {
                    trophies = null;
        ...
        /// <param name="callback">
        ///     A callback function accepting a single parameter,
        ///     a boolean indicating the success of this function.
        /// </param>
        ...
        {
            var parameters = new Dictionary<string, string> {{"trophy_id", id.ToString()}};
            Request.Get(Constants.ApiTrophiesRemove, parameters, response =>
            {
                // Update the cache.
