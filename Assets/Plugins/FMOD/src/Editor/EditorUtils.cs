    ...
    internal class EditorUtils : MonoBehaviour
    {
        ...
        public static void ValidateSource(out bool valid, out string reason)
        {
            ...
            if (settings.HasSourceProject)
            {
                ...
                var buildFolder = Path.Combine(projectFolder, BuildFolder);
        ...
        private static void OnlineManual()
        {
        ...
        {
            GetScriptOutput("nameConflict = false;");
            GetScriptOutput(string.Format("checkFunction = function(val) {{ nameConflict |= val.name == \"{0}\"; }};",
                eventName));
            GetScriptOutput(string.Format("studio.project.lookup(\"{0}\").items.forEach(checkFunction, this); ",
                folderGuid));
            var conflictBool = GetScriptOutput("nameConflict;");
        ...
        public static string CreateStudioEvent(string eventPath, string eventName)
        {
            ...
            for (var i = 0; i < folders.Length; i++)
            {
                ...
                GetScriptOutput("guid = \"\";");
                GetScriptOutput(string.Format(
                    "findFunc = function(val) {{ guid = val.isOfType(\"EventFolder\") && val.name == \"{0}\" ? val.id : guid; }};",
                    folders[i]));
                GetScriptOutput(string.Format("studio.project.lookup(\"{0}\").items.forEach(findFunc, this);",
                    folderGuid));
                folderGuid = GetScriptOutput("guid;");
                if (folderGuid == "")
                {
                    GetScriptOutput("folder = studio.project.create(\"EventFolder\");");
                    ...
                    folderGuid = GetScriptOutput("folder.id;");
                }
            }
            ...
            var eventGuid = GetScriptOutput("event.id;");
            return eventGuid;
        }
